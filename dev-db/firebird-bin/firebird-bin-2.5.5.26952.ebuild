...
MY_SRC_URI="http://planetexpress.ddnsking.com:9080/files/gentoo/distfiles"

DESCRIPTION="A relational database offering many ANSI SQL:2003 and some SQL:2008 features"
HOMEPAGE="http://www.firebirdsql.org/"
SRC_URI="
	amd64? (
		!superserver? ( ${MY_SRC_URI}/${PN}-cs-${PV}-x86_64.tar.xz )
		superserver? ( ${MY_SRC_URI}/${PN}-ss-${PV}-x86_64.tar.xz )
	)

	x86? (
		!superserver? ( ${MY_SRC_URI}/${PN}-cs-${PV}-x86.tar.xz )
		superserver? ( ${MY_SRC_URI}/${PN}-ss-${PV}-x86.tar.xz )
	)
"
